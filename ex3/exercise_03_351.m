...
% sigmafactor = 0.7; % this is the old value
% sigmafactor = 7; % this is a higher value
sigmafactor =logspace(-2,2,4);
sigmafactor = sigmafactor(4);
...
figure(3); colormap(gray); eigv_img=zeros(ne,dim);  
...
% lpcs = length(npcs);
% 
% figure(2); colormap('gray');title('Denosing using linear PCA');
% 
% for k=1:lpcs;
%  nb_pcs=npcs(k); 
